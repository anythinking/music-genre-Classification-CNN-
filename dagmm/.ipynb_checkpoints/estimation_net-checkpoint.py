    ...
    def inference(self, z, dropout_ratio=None):
        ...
        with tf.variable_scope("EstNet"):
            ...
            for size in self.hidden_layer_sizes[:-1]:
                n_layer += 1
                z = tf2.keras.layers.Dense(size, activation=self.activation,
                    name="layer_{}".format(n_layer))(z)
                

                
                if dropout_ratio is not None:
                    z = tf2.keras.layers.Dropout(dropout_ratio,
                        name="drop_{}".format(n_layer))(z)

            # Last layer uses linear function (=logits)
            size = self.hidden_layer_sizes[-1]
            logits = tf2.keras.layers.Dense(size, activation=None, name="logits")(z)
